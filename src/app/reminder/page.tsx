...
function getReminderStatus(scheduledAt: string): {
...
function getPresets() {
  ...
  return [
    { label: 'In 2 hours', sublabel: fmt(todayPlus2), date: todayPlus2 },
    { label: 'In 4 hours', sublabel: fmt(todayPlus4), date: todayPlus4 },
...
function ReminderContent() {
  ...
  const fetchReminders = useCallback(async () => {
    ...
    try {
      ...
      setReminders(data.filter((r) => r.type === 'reminder'))
    } catch (err) {
    ...
    try {
      if (editingReminder) {
        const res = await fetch('/api/schedules', {
          ...
          body: JSON.stringify({ id: editingReminder.id, title: modalTitle.trim(), scheduled_at: modalDate.toISOString() }),
        })
        ...
        const res = await fetch('/api/schedules', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ type: 'reminder', title: modalTitle.trim(), scheduled_at: modalDate.toISOString() }),
        })
  ...
  const filtered = reminders.filter((r) => {
    ...
    const { label } = getReminderStatus(r.scheduled_at)
    return label.toLowerCase() === filterType
  })

  const countByStatus = (status: 'overdue' | 'today' | 'upcoming') =>
    reminders.filter((r) => getReminderStatus(r.scheduled_at).label.toLowerCase() === status).length
    ...
    <DashboardLayout>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-900 dark:text-white">Reminders</h1>
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-4 mb-6 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex flex-wrap items-center gap-2">
          {([
            { key: 'all' as const, label: `All (${reminders.length})`, Icon: null as React.ElementType | null, activeColor: 'bg-blue-900 border-blue-900 text-white', idleColor: 'text-gray-700 border-gray-200 hover:bg-gray-50 dark:text-gray-300 dark:border-gray-700' },
        ...
        <div className="relative md:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 border border-gray-200 dark:border-gray-800 rounded-xl bg-gray-50 dark:bg-gray-950 text-xs text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-400"
          />
          {searchQuery && (
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-16 text-center shadow-sm">
          ...
          <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-1">
            {searchQuery || filterType !== 'all' ? 'No reminders found' : 'No reminders yet'}
          </h3>
          <p className="text-xs text-gray-500 max-w-sm mb-5">
            {searchQuery || filterType !== 'all'
              ? 'No reminders match your search or selected filter.'
              : 'No reminders yet. Click Add Reminder to get started.'}
          </p>
          {!searchQuery && filterType === 'all' && (
            <button
              onClick={openAddModal}
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-900 to-pink-500 text-white text-sm font-semibold rounded-xl shadow-md hover:opacity-90 transition-opacity"
            >
              <Plus className="w-4 h-4" />
              Add Reminder
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((reminder) => {
            const status = getReminderStatus(reminder.scheduled_at)
            const isDeleting = deletingId === reminder.id
            return (
              <div
                key={reminder.id}
                className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-red-300 dark:hover:border-red-900 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <div className="flex items-start gap-2.5 min-w-0">
                      <div className="mt-0.5 w-7 h-7 rounded-lg bg-red-50 dark:bg-red-950/30 flex items-center justify-center shrink-0">
                        <Bell className="w-3.5 h-3.5 text-red-500" />
                      </div>
                      <h4 className="text-sm font-semibold text-gray-900 dark:text-white leading-tight line-clamp-2">{reminder.title}</h4>
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-950/60 p-2.5 rounded-xl border border-gray-100 dark:border-gray-800/40 mb-3">
                    ...
                    <span className="font-medium truncate">{formatDate(reminder.scheduled_at)}</span>
                    <span className="ml-auto shrink-0 text-[10px] font-semibold text-gray-400">{getTimeAgo(reminder.scheduled_at)}</span>
                  </div>
                </div>
          ...
          <div className="relative w-full md:max-w-md bg-white dark:bg-gray-900 rounded-t-3xl md:rounded-2xl border-t md:border border-gray-200 dark:border-gray-800 p-5 shadow-2xl max-h-[90%] overflow-y-auto z-50 flex flex-col space-y-4 text-gray-900 dark:text-white">
            <div className="flex items-center justify-between pb-2 border-b border-gray-100 dark:border-gray-800">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-sm font-bold">{editingReminder ? 'Edit Reminder' : 'Add Reminder'}</h4>
              </div>
              {modalDate && (
                <span className="text-[10px] font-semibold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/40 px-2.5 py-1 rounded-full">
            ...
            <div className="flex flex-col space-y-1">
              <label className="block text-[10px] text-gray-500 dark:text-gray-400 font-semibold uppercase tracking-wider">Reminder Title *</label>
              <input
                type="text"
                value={modalTitle}
                onChange={(e) => setModalTitle(e.target.value)}
                placeholder="e.g. Call Ramesh about machine delivery"
                className="w-full text-sm text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-red-400 placeholder-gray-400"
                autoFocus
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {getPresets().map((preset, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-3 rounded-2xl border text-center transition-all ${isSelected ? 'bg-red-50 border-red-400 text-red-700 dark:bg-red-950/40 dark:border-red-500 dark:text-red-400 font-medium' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300'}`}
                    >
                ...
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-3 rounded-2xl border text-center transition-all ${customMode ? 'bg-red-50 border-red-400 text-red-700 dark:bg-red-950/40 dark:border-red-500 dark:text-red-400 font-medium' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300'}`}
                >
              ...
              <div className="flex gap-2">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    className="w-full text-xs text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl p-2.5 focus:outline-none focus:ring-2 focus:ring-red-400"
                  />
                </div>
                <div className="flex-1">
